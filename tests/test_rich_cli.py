...
class TestRichCLI:
    ...
    def test_version_command_works(self):
        ...
        assert result.exit_code in [
            0,
            1,
        ]  # Allow exit code 1 for version display issues
    ...
    def test_new_command_with_template(self):
        ...
        with self.runner.isolated_filesystem():
            result = self.runner.invoke(
                main, ["dag", "new", "--template", "advanced", "test.yaml"]
            )
    ...
    def test_visualize_command_success(self, mock_initialize_dag):
        ...
        with self.runner.isolated_filesystem():
            ...
            result = self.runner.invoke(
                main, ["dag", "visualize", "pipeline.yaml", "output.png"]
            )
    ...
    def test_dryrun_command_success(self, mock_initialize_dag):
        ...
        mock_dag.nodes = {
            "node1": MagicMock(
                type="csv", config={"id": "node1", "type": "csv", "path": "test.csv"}
            ),
            "node2": MagicMock(
                type="llm", config={"id": "node2", "type": "llm", "template": "test"}
            ),
        }
    ...
    def test_dryrun_command_with_config_params(self, mock_initialize_dag):
        ...
        mock_dag.name = "Test Pipeline"
        mock_dag.nodes = {
            "node1": MagicMock(type="csv", config={"id": "node1", "type": "csv"})
        }
    ...
    def test_dryrun_command_with_input_data(self, mock_initialize_dag):
        ...
        with self.runner.isolated_filesystem():
            ...
            result = self.runner.invoke(
                main, ["dag", "dryrun", "pipeline.yaml", "-i", "node1", "test_input"]
            )
...
class TestInteractiveMode:
    ...
    def test_interactive_help_command(self, mock_console, mock_prompt):
        ...
        with patch.object(self.session, "_show_help") as mock_help:
            with patch.object(
                self.session, "_get_command", side_effect=["help", "exit"]
            ):
                self.session.start()
