...
class RichGroup(click.Group):
    ...
    def _show_usage(self, ctx):
        ...
        console.print(f"  [dim]$[/dim] [green]wordcel[/green] [cyan]COMMAND[/cyan] [yellow][OPTIONS][/yellow]")
        console.print(f"  [dim]$[/dim] [green]wordcel[/green] [cyan]--help[/cyan]")
        console.print(f"  [dim]$[/dim] [green]wordcel[/green] [cyan]COMMAND[/cyan] [cyan]--help[/cyan]")
    ...
    def _show_examples(self, ctx):
        ...
        if ctx.parent is None:  # Root command examples
            examples = [
                ...
                ("Visualize pipeline", "wordcel dag visualize my-pipeline.yaml output.png"),
    ...
    def _get_subcommand_examples(self, command_name: str) -> List[Tuple[str, str]]:
        ...
        examples_map = {
            "dag": [
                ("Create and run pipeline", "wordcel dag new test.yaml && wordcel dag execute test.yaml"),
                ("Run with custom input", "wordcel dag execute pipeline.yaml -i input_node 'Hello World'"),
...
class RichCommand(click.Command):
    ...
    def _show_parameters(self):
        ...
        if arguments:
            table = Table(title="Arguments", show_header=True, header_style="bold yellow")
    ...
    def _show_command_examples(self, command_path: str):
        ...
        examples_map = {
            ...
            "dag execute": [
                ...
                ("With input data", "wordcel dag execute pipeline.yaml -i node1 'hello world'"),
                ("Debug mode", "wordcel dag execute pipeline.yaml --verbose"),
                ("With secrets", "wordcel dag execute pipeline.yaml --secrets secrets.yaml"),
                ("Custom config", "wordcel dag execute pipeline.yaml -c model_name gpt-4o")
            ],
            "dag visualize": [
                ("PNG output", "wordcel dag visualize pipeline.yaml graph.png"),
                ("With custom nodes", "wordcel dag visualize pipeline.yaml graph.png --custom-nodes nodes.py")
            ]
        }
