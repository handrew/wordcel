...
"""
    ...
    def _get_command(self) -> str:
        ...
        if self.current_pipeline:
            pipeline_name = Path(self.pipeline_path).name if self.pipeline_path else "unsaved"
            prompt_text = f"[green]wordcel[/green]([cyan]{pipeline_name}[/cyan])[green]>[/green]"
    ...
    def _create_pipeline(self):
        ...
        console.print(Panel("[bold green]🆕 Create New Pipeline[/bold green]", border_style="green"))
        ...
        if os.path.exists(pipeline_name):
            overwrite = Confirm.ask(f"File {pipeline_name} exists. Overwrite?", default=False)
            if not overwrite:
...
"""
    ...
    def _edit_pipeline(self):
        ...
        if not self.pipeline_path:
            console.print("[yellow]No pipeline loaded. Create one first with 'new'[/yellow]")
            return
    ...
    def _show_status(self):
        ...
        console.print(Panel("[bold blue]📊 Session Status[/bold blue]", border_style="blue"))
